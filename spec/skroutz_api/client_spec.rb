...
describe SkroutzApi::Client do
  let(:client_id) { 'a valid client_id' }
  let(:client_secret) { 'a valid client_secret' }
  let(:config_options) { {} }

  subject(:client) { SkroutzApi::Client.new(client_id, client_secret, config_options) }
  ...
  describe '#config' do
    ...
    context 'when none is supplied' do
      it { is_expected.to eq(SkroutzApi::Default.to_hash) }
    end
